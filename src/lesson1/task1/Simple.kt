...
fun lengthInMeters(sagenes: Int, arshins: Int, vershoks: Int): Double =
        (sagenes * 48 + arshins * 48.0 / 3.0 + vershoks) * 4.445 / 100.0
...
fun angleInRadian(deg: Int, min: Int, sec: Int): Double =
        PI / 180.0 * (deg + (min + sec / 60.0) / 60.0)
...
fun travelMinutes(hoursDepart: Int, minutesDepart: Int, hoursArrive: Int, minutesArrive: Int): Int =
        (hoursArrive - hoursDepart - (if (minutesArrive > minutesDepart) 1 else 0)) * 60 + (if (minutesArrive > minutesDepart) 60 else 0) + minutesArrive - minutesDepart
...
fun numberRevert(number: Int): Int =
        firstDigit(number) * 100 + secondDigit(number) * 10 + thirdDigit(number)
